...
BASE_PRECISION_MUL: constant(uint256[BASE_N_COINS]) = [10000000000, 1, 1000000000000]
...
def _xp(vp_rate: uint256[N_COINS]) -> uint256[N_COINS]:
    ...
    result[0] = vp_rate[0]
    result[MAX_COIN] = vp_rate[MAX_COIN]  # virtual price for the metacurrency
    for i in range(N_COINS):
...
@pure
@internal
def _xp_mem(vp_rate: uint256[N_COINS], _balances: uint256[N_COINS]) -> uint256[N_COINS]:
    result: uint256[N_COINS] = RATES
    result[0] = vp_rate[0]
    result[MAX_COIN] = vp_rate[MAX_COIN]  # virtual price for the metacurrency
    for i in range(N_COINS):
...
def get_dy_underlying(i: int128, j: int128, dx: uint256) -> uint256:
    ...
    if base_i < 0:
        ...
        if base_j < 0:
            ...
            x = Curve(_base_pool).calc_token_amount(base_inputs, True) * vp_rate[MAX_COIN] / PRECISION
    ...
    if base_j < 0:
        ...
        dy = Curve(_base_pool).calc_withdraw_one_coin(dy * PRECISION / vp_rate[MAX_COIN], base_j)
...
@view
@internal
def _calc_withdraw_one_coin(_token_amount: uint256, i: int128, vp_rate: uint256[N_COINS]) -> (uint256, uint256, uint256):
    ...
    rates: uint256[N_COINS] = RATES
    rates[0] = vp_rate[0]
    rates[MAX_COIN] = vp_rate[MAX_COIN]
